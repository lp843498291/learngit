<template>
    <div class="shop-cart">
        ...
        <div class="shop-cart-wrap">
            ...
            <!-- 左侧盒子 -->
            <div @click="changeVisible"
                class="left-box">
                <div class="icon-wrap"
                    :class="{active: totalCount > 0}">
                    <i class="iconfont icon-gouwucheman"></i>
                    <span class="zs">{{ totalCount }}</span>
                </div>
                <span class="txt">{{ totalCount > 0 ? '￥' + totalPrice : '未选中商品' }}</span>
            </div>

            <!-- 右侧 -->
            <div class="right-box">
                <button v-if="totalPrice > 20"
                    class="jiesuan">去结算</button>
                ...
                <ul class="list">
                    ...
                    <li v-for="item in shopListData"
                        ...
                        <div class="control-box">
                            <!-- + - 组件 -->
                            <button class="btn1"
                                @click="changeCount(item.name, -1)">-</button>
                            <span>{{ item.count }}</span>
                            <button @click="changeCount(item.name, 1)"
                                class="btn2">+</button>
                        </div>
                    </li>
                </ul>
